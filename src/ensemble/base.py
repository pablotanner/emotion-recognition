...
class BaseRandomForestModel(object):
    ...
    def _fit(self, model, dataset, convert_dtype, broadcast_data, class_weight=None):
        ...
        for idx, (worker, worker_data) in enumerate(
            ...
            futures.append(
                self.client.submit(
                    ...
                    convert_dtype,
                    class_weight,  # Pass the class_weight parameter here
                    workers=[worker],
                    pure=False,
                )
            )
...
def _func_fit(model, input_data, convert_dtype, class_weight=None):
    ...
    if class_weight is not None:
        ...
        if class_weight == 'balanced':
            ...
            for cls in unique_classes:
                class_weight_dict[cls] = total_samples / (len(unique_classes) * class_counts[cls])
        else:
    ...
    return model.fit(X, y, convert_dtype=convert_dtype, sample_weight=sample_weights)
