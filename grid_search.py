...
# Checkpoint logic, save and load checkpoints (which params have been tried)
...
# Experiments for finding optimal (tuned) classifier for the different feature types
parser = argparse.ArgumentParser(description='Finding optimal classifier for different feature types')
...
if __name__ == '__main__':
    ...
    logger.info(f'Running experiments for feature type {args.feature}')

    # I sometimes use pdm as a shorthand for nonrigid_face_shape
    if args.feature == 'pdm':
    ...
    # This loads the feature types with minimal preprocessing steps
    ...
    # This loads the feature types with PCA (n=100) from information fusion experiments
    ...
    # Store shape for the nn classifiers
    ...
    parameters = {
        'SVC': {'C': [0.1, 1, 10], 'kernel': ['rbf', 'poly'], 'probability': [True], 'class_weight': ['balanced']},
    ...
    for clf_name, clf_class in classifiers.items():
        ...
        for params in param_grid:
            if params in tried_params:
                continue
            # Different handling for different classifiers
            if clf_name == 'NN':
                clf = SequentialNN(input_dim=X_shape, **params)
                clf.compile(optim.Adam(clf.parameters(), lr=0.001))
            elif clf_name == 'MLP':
                clf = MLP(input_size=X_shape, num_classes=8, **params)
            else:
            ...
            try:
                # Different classifiers require different data types
                failed = False
        ...
        """
        This code would remember the best classifier for each type and evaluate it on the test set,
        however it would frequently cause memory crashes (because I didn't add error handling and since 
        the results can also be seen in the logs, I decided to comment it out)
